        ...
        {treeTalents.map(talent => {
          ...
          return (
            <button 
              key={talent.id}
              id={`talent-${talent.id}`}
              className={`absolute w-11 h-11 z-10 group rounded transition-all duration-200 outline-none before:absolute before:-inset-2.5 before:content-[''] before:z-0 ${breathingClass} ${canAddPoint(talent, treeTalents) ? 'hover:scale-105' : ''}`}
              style={{
                left: `${26 + talent.col * 72}px`,
                top: `${26 + talent.row * 80}px`
              }}
